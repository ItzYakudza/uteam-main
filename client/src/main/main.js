...
function createWindow() {
    ...
    // Обработчик закрытия окна - сворачиваем в трей вместо полного закрытия
...
ipcMain.on('window:close', () => {
    // Сначала скрываем окно (сворачиваем в трей)
    if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.hide();
        if (mainWindow.isMaximized()) {
            mainWindow.unmaximize();
    ...
    try {
            ...
            const download = (url, attempts = 0) => {
                ...
                const request = protocol.get(url, options, (response) => {
                    ...
                    if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
                        download(response.headers.location, attempts + 1);
                        return;
                    }
        ...
        const findExe = (dir) => {
            try {
                ...
                console.error('Error finding exe:', e);
            }
            return null;
        };
        ...
        if (!executablePath) {
            throw new Error('No executable found in game archive');
        }
